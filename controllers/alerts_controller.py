...
class AlertsController:
    ...
    def get_windows_agent_id(self):
        """Get the ID of the configured Windows agent"""
        try:
            # Check if we have a saved agent ID
            if self.wazuh_config.agent_id:
                self.windows_agent_id = self.wazuh_config.agent_id
                print(f"Using configured agent ID: {self.windows_agent_id} ({self.wazuh_config.agent_name})")

                # Verify the agent is still active
                if self.get_wazuh_token():
                    response = self.fetch_data('agents', {'agents_list': self.windows_agent_id})
                    if response and 'data' in response:
                        agents = response['data'].get('affected_items', [])
                        if agents and agents[0].get('status') == 'active':
                            return self.windows_agent_id
                        else:
                            print(f"Warning: Configured agent {self.wazuh_config.agent_name} is not active")

            # Fallback to searching for active Windows agents
            if self.windows_agent_id:
                return self.windows_agent_id

            # If no active Windows agent found in this query, continue to next
            print("No active Windows agents found in this query")
    ...
    def update_alerts(self):
        ...
        # 2) Choose date filter window if desired
        start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d")

        # 3) Call Wazuh API with limit & offset
        params = {
            'limit': self.limit,
            'offset': self.offset,
            'q': f'date>{start_date}',
        ...
        response = self.fetch_data(f'syscheck/{agent_id}', params)
        ...
        # 4) Transform into your table rows
        alerts = []
        seen_ids = set()
        date_counts = {}
        for item in items:
    ...
    def check_recent_security_events(self):
        ...
        if not windows_id or not self.get_wazuh_token():
            return
    ...
    def check_agent_status(self):
        ...
        # Get agent details
        response = self.fetch_data(f'agents', {'agents_list': windows_id})
        ...
        stats_response = self.fetch_data(f'agents/{windows_id}/stats/analysisd', {})

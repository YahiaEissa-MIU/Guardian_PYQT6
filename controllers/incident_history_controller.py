...
class IncidentHistoryController(QObject):
    ...
    def request_filter(self, filter_type, filter_value):
        """Request filtering with debounce to prevent rapid updates"""
        self.pending_filter = (filter_type, filter_value)
        self.filter_timer.stop()
        self.filter_timer.start(300)  # 300ms delay

    def _apply_filter(self):
        """Apply the pending filter"""
        if self.pending_filter and self.model:
            filter_type, filter_value = self.pending_filter
            filtered_incidents = self.model.get_incidents(filter_type, filter_value)
            self.incidents_updated.emit(filtered_incidents)
            self.pending_filter = None

    def get_incident_cves(self, incident_index):
        """Get CVEs for a specific incident by index"""
        if self.model:
            return self.model.get_cves_for_incident(incident_index)
        return []
    ...
    def _generate_statistics_table(self, incidents):
        """Generate statistics table with wrapped item names"""
        from reportlab.platypus import Paragraph
        from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
        ...
        for workflow, count in sorted(workflows.items(), key=lambda x: x[1], reverse=True):
            ...
            data.append([
                'Workflow',
                Paragraph(workflow, wrap_style),
                str(count),
                percentage
            ])

        # Create and style table
        table = Table(data, colWidths=[1.5 * inch, 3.3 * inch, 0.8 * inch, 0.9 * inch])
    ...
    def _generate_incident_table(self, incidents, color=None):
        ...
        for inc in incidents[:20]:
            data.append([
                inc.get('Date', '')[:16],
                Paragraph(inc.get('Incident', '') or '—', wrap),
                Paragraph(inc.get('Action', '') or '—', wrap),
                Paragraph(inc.get('CVE', '') or '—', wrap)
            ])
    ...
    def cleanup(self):
        ...
        try:
            ...
            logging.error(f"Error during filter cleanup: {e}")
        try:

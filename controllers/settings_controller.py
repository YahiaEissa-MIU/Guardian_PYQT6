...
class SettingsController(QObject):
        ...
        try:
            ...
            if os.path.exists(wazuh_config_path):
                ...
                if self.wazuh_config.is_configured:
                    ...
                    # Add agent info
                    self.wazuh_agent_name = self.wazuh_config.agent_name
                    self.wazuh_agent_id = self.wazuh_config.agent_id
                    print(
                        f"Wazuh config loaded successfully. Agent: {self.wazuh_agent_name} (ID: {self.wazuh_agent_id})")
    ...
    def get_current_settings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "wazuh_agent_name": getattr(self, 'wazuh_agent_name', ''),
            "wazuh_agent_id": getattr(self, 'wazuh_agent_id', ''),
            "shuffle_url": self.shuffle_url,
            "shuffle_api_key": self.shuffle_api_key,
            "shuffle_workflows": self.shuffle_workflows
        }

    def save_settings(self, settings: Dict[str, Any]) -> bool:
        """Saves Wazuh settings after validation"""
        try:
            # 1) pull everything out of `settings` first
            ...
            # Pull agent info out of settings
            agent_name = settings.get("wazuh_agent_name", "")
            agent_id = settings.get("wazuh_agent_id", "")

            # 2) now update your controller’s fields so get_current_settings() returns them
            self.wazuh_agent_name = agent_name
            ...
            if new_config.save_to_file(config_path):
                ...
                if new_config.validate_connection():
                    ...
                    QMessageBox.information(None, "Success",
                                            f"Settings saved and connection verified!\nAgent: {agent_name}" if agent_name else "Settings saved and connection verified!")
    ...
    def get_available_agents(self):
        ...
        try:
            ...
            # Authenticate first
            base_url = temp_config.get_base_url()
            print(f"Fetching agents from: {base_url}")

            auth_response = requests.get(
                f"{base_url}/security/user/authenticate",
                auth=HTTPBasicAuth(self.wazuh_username, self.wazuh_password),
                verify=False,
                timeout=5
            )
            ...
            if agents_response.status_code == 200:
                agents_data = agents_response.json()['data']['affected_items']
                ...
                for agent in agents_data:
                    if agent['id'] != '000':  # Skip manager
                        formatted_agents.append({
                            ...
                            'name': agent['name'],
                            'status': agent['status'],
                            'os': agent.get('os', {}).get('platform', 'unknown'),
                            'last_keep_alive': agent.get('lastKeepAlive', 'Never')
                        })

                # Sort by status (active first) then by name
                formatted_agents.sort(key=lambda x: (x['status'] != 'active', x['name']))

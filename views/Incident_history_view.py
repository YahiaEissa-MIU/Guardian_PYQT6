...
class IncidentHistoryView(QWidget):
    ...
    def show_cve_info(self, incident, incident_index):
        ...
        # Add extraction source information if available
        if 'Details' in incident:
            try:
                details = json.loads(incident['Details'])
                if 'cve_sources' in details:
                    html += "<h4>CVE Extraction Sources:</h4>"
                    html += "<ul style='margin: 10px 0;'>"
                    for source, cves in details['cve_sources'].items():
                        if cves:
    ...
    def export_with_feedback(self, format_type):
        """Export with immediate user feedback"""
        ...
        QMessageBox.information(self, "Export Started", f"Exporting to {format_type.upper()}... Please wait.")
    ...
    def on_generate_report(self):
        ...
        # Add this line for immediate feedback
        QMessageBox.information(self, "Report Generation", "Generating report... Please wait.")

...
class SettingsView(QWidget):
    ...
    def create_api_connection_settings(self, parent_layout):
        ...
        # --- ADD the agent list widget (modeled on workflow list) ---------------
        # Label
        agent_label = QLabel("Available Agents:")
        agent_label.setStyleSheet("color: #2c3e50; font-weight: bold;")

        # List widget
        self.agent_list = QListWidget()
        self.agent_list.setSelectionMode(QListWidget.SelectionMode.SingleSelection)
        ...
        # “Selected:” label
        self.selected_agent_label = QLabel("Selected: None")
        self.selected_agent_label.setStyleSheet("color: #7f8c8d; font-size: 12px;")
        self.selected_agent_label.setWordWrap(True)

        # build a vertical box so the list + label sit neatly under the form grid
        agent_box = QVBoxLayout()
        agent_box.setContentsMargins(0, 0, 0, 0)
        agent_box.setSpacing(5)
        agent_box.addWidget(agent_label)
        agent_box.addWidget(self.agent_list)
        agent_box.addWidget(self.selected_agent_label)

        # agent_box -> a dummy QWidget → place it in the form grid (full row span)
        agent_box_widget = QWidget()
        agent_box_widget.setLayout(agent_box)
        form_layout.addWidget(agent_box_widget, 3, 0, 1, 2)
        ...
        # ADD FETCH AGENTS BUTTON
        self.fetch_agents_btn = QPushButton("Fetch Agents")
        self.fetch_agents_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        self.fetch_agents_btn.setFixedSize(120, 32)
        self.agent_list.itemSelectionChanged.connect(self.update_selected_agent_label)
        self.fetch_agents_btn.clicked.connect(self.on_fetch_agents)
        ...
        button_layout.addWidget(self.fetch_agents_btn)
    ...
    def update_selected_agent_label(self):
        """Show current agent selection."""
        item = self.agent_list.currentItem()
        if item:
            self.selected_agent_label.setText(f"Selected: {item.text()}")
        else:
            self.selected_agent_label.setText("Selected: None")
    ...
    def on_save_wazuh_settings(self):
        ...
        # grab the single-selected item from agent list
        ...
        if self.controller:
            settings = {
                ...
                "wazuh_agent_name": selected_agent["name"] if selected_agent else None,
                "wazuh_agent_id": selected_agent["id"] if selected_agent else None,
            }
    ...
    @pyqtSlot()
    def on_fetch_agents(self):
        """Fetch available agents from Wazuh"""
        print("Fetching Wazuh agents...")

        # Validate inputs
        url = self.wazuh_url_input.text().strip()
        username = self.wazuh_username_input.text().strip()
        password = self.wazuh_password_input.text().strip()

        if not all([url, username, password]):
            QMessageBox.warning(self, "Warning", "Please enter Wazuh URL, username, and password first.")
            return

        # Disable button during fetch
        self.fetch_agents_btn.setEnabled(False)
        self.fetch_agents_btn.setText("Fetching...")

        try:
            if self.controller:
                # Update controller with current values temporarily
                self.controller.wazuh_url = url
                self.controller.wazuh_username = username
                self.controller.wazuh_password = password
                ...
                if agents:
                    for agent in agents:
                        text = f"{agent['name']} (ID: {agent['id']}) - {agent['status']}"
                        item = QListWidgetItem(text)
                        item.setData(Qt.ItemDataRole.UserRole, agent)
                        self.agent_list.addItem(item)

                        # auto-select previously saved agent
                        if hasattr(self.controller, 'wazuh_agent_name') \
                                and agent['name'] == self.controller.wazuh_agent_name:
                            item.setSelected(True)
                    self.update_selected_agent_label()
                    QMessageBox.information(self, "Success", f"Found {len(agents)} agents")
                else:
                    QMessageBox.warning(self, "Warning", "No agents found or connection failed.")

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to fetch agents: {str(e)}")
        finally:
            self.fetch_agents_btn.setEnabled(True)
            self.fetch_agents_btn.setText("Fetch Agents")
    ...
    def update_fields(self, settings):
        ...
        try:
            ...
            # Update agent dropdown if we have a saved agent
            if "wazuh_agent_name" in settings and settings["wazuh_agent_name"]:
                agent_name = settings["wazuh_agent_name"]
                agent_id = settings.get("wazuh_agent_id", "")
                # Add the saved agent to dropdown if not already there
                if hasattr(self, 'agent_list'):
                    self.agent_list.clear()
                    item = QListWidgetItem(f"{agent_name} (ID: {agent_id})")
                    item.setData(Qt.ItemDataRole.UserRole, {'name': agent_name, 'id': agent_id})
                    item.setSelected(True)
                    self.agent_list.addItem(item)
                    self.update_selected_agent_label()
